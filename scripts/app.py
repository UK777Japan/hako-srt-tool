...
TRANSCRIBE_STEP_INFO = [
    ("[0]",              5,  "ステップ 1/2: 音声変換中 (ffmpeg)"),
    ("[1]",             15,  "ステップ 2/2: Whisper 音声認識を準備中"),
    ("モデル",          25,  "ステップ 2/2: Whisper モデルをロード中（初回は数分かかります）"),
    ("Transcribe",      30,  "ステップ 2/2: 音声認識中（しばらくお待ちください）"),
    ("キャッシュを使用", 30,  "ステップ 2/2: Whisper キャッシュを使用"),
    ("認識セグメント数", 85,  "ステップ 2/2: 音声認識完了"),
    ("[完了]",         100,  "完了！"),
]
...
# ---------------------------------------------------------------------------
for key in [
    "srt_bytes", "txt_bytes", "csv_bytes", "run_success", "run_error", "run_elapsed",
    "tr_srt_bytes", "tr_txt_bytes", "tr_csv_bytes", "tr_run_success", "tr_run_error", "tr_run_elapsed",
]:
...
# タブ
# ---------------------------------------------------------------------------
tab1, tab2 = st.tabs(["🔖 ハコ割り生成", "📝 文字起こし（原稿なし）"])
...
with tab2:
    st.markdown(
        "音声ファイルをアップロードするだけで、原稿なしで文字起こしできます。"
        "Whisper が自動でセグメントに区切り、SRT・TXT・CSV を出力します。"
    )

    tr_audio = st.file_uploader(
        "音声・映像ファイル",
        type=["mp3", "wav", "m4a", "aac", "flac", "ogg", "mpg", "mpeg", "mp4", "ts"],
        help="音声・動画ファイル（mp4/mpg/ts は音声トラックを使用）。上限 10GB。",
        key="tr_audio",
    ...
    tr_run_btn = st.button(
        "▶ 文字起こしを実行",
        type="primary",
        disabled=(tr_audio is None),
        use_container_width=True,
    )

    if tr_run_btn:
        for key in ["tr_srt_bytes", "tr_txt_bytes", "tr_csv_bytes",
                    "tr_run_success", "tr_run_error", "tr_run_elapsed"]:
            st.session_state[key] = None

        OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
        TR_TMP_DIR = Path(tempfile.mkdtemp(prefix="tr_"))
        tr_timestamp = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
        tr_log_lines: list[str] = []

        with st.status("文字起こし中...", expanded=True) as tr_status:
            tr_prog    = st.progress(0, text="準備中...")
            tr_log_area = st.empty()
            tr_cur_pct  = [0]

            def tr_log(msg: str):
                tr_log_lines.append(msg)
                tr_log_area.code("\n".join(tr_log_lines), language=None)
                for keyword, pct, label in TRANSCRIBE_STEP_INFO:
                    if keyword in msg:
                        tr_cur_pct[0] = pct
                        tr_prog.progress(pct, text=label)
                        break

            try:
                tr_audio_path = TR_TMP_DIR / tr_audio.name
                tr_audio_path.write_bytes(tr_audio.read())

                tr_log(f"音声ファイル: {tr_audio.name}")
                ...
                try:
                    ...
                    tr_device = "cpu"
                tr_log(f"デバイス: {tr_device}")

                import sys
                sys.path.insert(0, str(Path(__file__).parent))
                from sync_srt import transcribe_only

                tr_start = time.time()
                tr_srt_path, tr_txt_path, tr_csv_path = transcribe_only(
                    audio_path=tr_audio_path,
                    output_dir=OUTPUT_DIR,
                    model_name=model_name,
                    fps=int(fps),
                    global_offset=float(global_offset),
                ...
                tr_log(f"処理時間: {tr_elapsed:.1f} 秒")
                tr_log("結果: 成功")

                tr_prog.progress(100, text="完了！")
                st.session_state["tr_srt_bytes"]   = tr_srt_path.read_bytes()
                st.session_state["tr_txt_bytes"]   = tr_txt_path.read_bytes()
                st.session_state["tr_csv_bytes"]   = tr_csv_path.read_bytes()
                st.session_state["tr_run_success"] = True
                st.session_state["tr_run_elapsed"] = tr_elapsed
                tr_status.update(label=f"文字起こし完了！（{tr_elapsed:.1f} 秒）",
                                 state="complete", expanded=False)

            except Exception as e:
                tb_str = traceback.format_exc()
                tr_log(f"結果: エラー - {e}")
                st.session_state["tr_run_error"] = tb_str
                tr_prog.progress(tr_cur_pct[0], text="エラーが発生しました")
                tr_status.update(label="エラーが発生しました", state="error", expanded=True)

            finally:
                shutil.rmtree(TR_TMP_DIR, ignore_errors=True)
                OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
                tr_entry = f"=== {tr_timestamp} [文字起こし] ===\n" + "\n".join(tr_log_lines) + "\n\n"
                with LOG_FILE.open("a", encoding="utf-8") as f:
                    f.write(tr_entry)

    if st.session_state.get("tr_run_success"):
        tr_elapsed = st.session_state["tr_run_elapsed"]
        st.success(f"文字起こし完了！（{tr_elapsed:.1f} 秒）")
        tdl1, tdl2, tdl3 = st.columns(3)
        with tdl1:
            st.download_button(
                "⬇ transcribe.srt をダウンロード",
                data=st.session_state["tr_srt_bytes"],
                file_name="transcribe.srt",
                mime="text/plain",
                use_container_width=True,
            )
        with tdl2:
            st.download_button(
                "⬇ transcribe.txt をダウンロード",
                data=st.session_state["tr_txt_bytes"],
        ...
        with tdl3:
            st.download_button(
                "⬇ transcribe.csv をダウンロード",
                data=st.session_state["tr_csv_bytes"],
                file_name="transcribe.csv",
                mime="text/csv",
                use_container_width=True,
            )

    if st.session_state.get("tr_run_error"):
        st.error("エラーが発生しました")
        st.markdown("**エラー詳細**（右上のコピーボタンで全文コピーできます）")
        st.code(st.session_state["tr_run_error"], language=None)
